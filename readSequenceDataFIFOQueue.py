...
#     )
     ...
     padding_q = tf.PaddingFIFOQueue(
         ...
         shapes = [[None], [None, 4096]])
     # enqueue the examples
     # padding_q = tf.PaddingFIFOQueue()
     # padding_q.enqueue()
     enqueue_op = padding_q.enqueue([sequence["labels"], sequence["tokens"]])
     qr = tf.train.QueueRunner(padding_q, [enqueue_op])
     tf.train.add_queue_runner(qr)
     
     # Dequeue padded data
     batch_data = padding_q.dequeue_many(60, name=None)
     ...
     res = tf.contrib.learn.run_n({"sequence": batch_data}, feed_dict=None, 
                                  restore_checkpoint_path=None, n = 3)
